...
if __name__ == '__main__':
    ...
    times = torch.linspace(0, 1, 7)
    # (2, 1) are batch dimensions. 7 is the time dimension (of the same length as t). 3 is the channel dimension.
    X_before_nan = torch.rand(1, 1, 7, 1)
    X = X_before_nan.clone()
    X[0,0,3,0] = torch.tensor(float('nan'))
    X[0,0,5,0] = torch.tensor(float('nan'))
    delete_tp = [3, 5]
    for _ in delete_tp:
        X[0, 0, _, 0] = torch.tensor(float('nan'))
    coeffs = natural_cubic_spline_coeffs(times, X)
    # ...at this point you can save the coeffs, put them through PyTorch's Datasets and DataLoaders, etc...
    spline = NaturalCubicSpline(times, coeffs)
    t = torch.tensor(0.4)
    # will be a tensor of shape (2, 1, 3), corresponding to batch and channel dimensions
    out = spline.derivative(t)
    print(out.shape, out, spline.evaluate(t))
    import matplotlib
    from matplotlib import pyplot as plt
    times_inter = torch.linspace(0, 1, 14)
    X_inter = torch.stack([spline.evaluate(t) for t in times_inter]).squeeze().cpu().numpy()
    X_d_inter = torch.stack([spline.derivative(t) for t in times_inter]).squeeze().cpu().numpy()
    plt.plot(times_inter, X_inter, label='evaluation')
    plt.plot(times_inter, X_d_inter, label='derivative')
    plt.plot(times, X_before_nan.squeeze().cpu().numpy(), label='before_delete')
    plt.scatter(times[delete_tp], X_before_nan[0,0,delete_tp,0].numpy(), s=10, label='Deleted points')
    plt.legend()
    # utils.makedirs('inter_test')
    plt.savefig('inter_test/test.png')
